...
import { useGetCategoriesQuery, useDeleteCategoryMutation, type Err } from '../services/flashcardApi';
...
const CategorySidebar: React.FC<CategorySidebarProps> = ({ 
  ...
  const isServerDisconnected = error && typeof error === 'object' &&
    (('message' in (error as Err) && (error as Err).message === 'SERVER_DISCONNECTED') || 
    ('status' in (error as Err) && (error as Err).status === 0));
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      // If a dialog is open and the click is inside it, don't close the menu.
      const dialogRoot = document.querySelector('.fixed.inset-0.z-50');
      if (dialogRoot && dialogRoot.contains(target)) {
        return;
      }
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    if (window.confirm('Are you sure you want to delete this category? All flashcards in this category will also be affected.')) {
      try {
  ...
  const totalFlashcards = categories.reduce((sum: number, cat: Category) => sum + cat.flashcardCount, 0);
  ...
  return (
    <div className={`${isCollapsed ? 'w-16' : 'w-80'} bg-white border-r border-gray-200 flex flex-col transition-all duration-300`}>
      ...
      <div className="p-4 border-b border-gray-100">
        ...
        >
          <div className="flex items-center">
            <div className={`w-8 h-8 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-lg flex items-center justify-center ${isCollapsed ? '' : 'mr-3'}`}>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
          ...
          {showSkeleton ? (
            ...
            !isCollapsed && (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-600 mb-2">Categories unavailable</p>
                <p className="text-xs text-gray-500">Server connection failed</p>
              </div>
            )
          ) : (
            // Actual categories
            ...
            <div key={category.id} className="relative group">
              ...
              >
                <div className="flex items-center min-w-0">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCollapsed ? '' : 'mr-3'}`} 
        ...
        {!showSkeleton && !isServerDisconnected && categories.length === 0 && !isCollapsed && (
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div>
              <p className="text-2xl font-bold text-blue-600">{totalFlashcards}</p>
