...
interface SimplePracticeDrillViewerProps {
  questionIds?: string[];
  data?: PracticeDrillData;
  onSubmit?: (answers: Record<number, string>) => void;
}

export const SimplePracticeDrillViewer = ({
  questionIds,
  data: providedData,
  onSubmit,
}: SimplePracticeDrillViewerProps) => {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 text-lg">
          Phần {currentPart.partName} chưa được hỗ trợ.
        </p>
      </div>
    );
  };
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <Card className="p-8">
          <p className="text-gray-600">Không có dữ liệu câu hỏi.</p>
        </Card>
      </div>
    );
  }
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <span>
                    {totalQuestions} câu hỏi • {data.parts.length} phần
                  </span>
                  {!isSubmitted && (
                    <>
                      <span>•</span>
