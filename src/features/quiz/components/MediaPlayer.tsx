...
export const MediaPlayer = ({
  ...
  const playAudio = (audioUrl: string, id: string) => {
    if (currentAudioElement) {
      ...
      dispatch(
        setIsPlayingAudio({ audioId: currentAudioElement.id, isPlaying: false })
      );
    }
  ...
  return (
    <div
      className={cn(
        'flex items-center space-x-2 p-2 bg-muted/30 rounded-lg',
        className
      )}
    >
      ...
      <Button
        ...
        size="sm"
        onClick={() =>
          isPlayingAudio[audioId]
            ? stopAudio(audioId)
            : playAudio(audio, audioId)
        }
