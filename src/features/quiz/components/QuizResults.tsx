...
import { resetQuizInterface, setTimeLeft, setReviewMode } from '../slices/quizSlice';
...
export const QuizResults = ({ onClose, score, totalQuestions, activeQuiz }: QuizResultsProps) => {
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl max-h-screen overflow-y-auto">
        <Card className="mx-auto p-6 shadow-lg rounded-xl bg-gradient-to-br from-purple-50 to-indigo-50">
          <CardHeader className="relative text-center border-b pb-4 mb-6 border-gray-200">
              ...
              <CardTitle className="text-3xl font-extrabold text-indigo-800">Quiz Results</CardTitle>
              <Button variant="ghost" size="sm" onClick={onClose} className="absolute right-0 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                <X className="h-5 w-5" />
              </Button>
          </CardHeader>
            ...
            <div className="text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg text-gray-600">
                  You scored <span className="font-semibold text-indigo-600">{score}</span> out of <span className="font-semibold text-indigo-600">{totalQuestions}</span> questions correctly
                </p>
              </div>
            </div>
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        ...
                        isCorrect ? "border-green-300 bg-green-50 hover:bg-green-100" : "border-red-300 bg-red-50 hover:bg-red-100"
                      )}
                      onClick={() => dispatch(setReviewMode({ isReviewMode: true, reviewQuestionIndex: idx }))}
                    >
                      ...
                      <p className="text-sm text-gray-600 line-clamp-2">{q.question.text}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        Your answer: <span className="font-semibold">{selectedOption?.text || 'Not answered'}</span>
                      </p>
                    </div>
                  );
                })}
