...
const EnhancedAudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  useEffect(() => {
    if (audioRef.current && externalCurrentTime !== undefined) {
      const timeDifference = Math.abs(audioRef.current.currentTime - externalCurrentTime / 1000);
      if (timeDifference > 0.1) { // Only seek if difference is significant
  ...
  const skipForward = useCallback(() => {
    const newTime = Math.min(playerState.currentTime + 5000, playerState.duration);
    seekTo(newTime);
    ...
    <div className={`bg-gray-50 border-b border-gray-200 p-4 ${className}`}>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="text-sm text-gray-600 font-mono min-w-[100px]">
          {formatTime(playerState.currentTime / 1000)} / {formatTime(playerState.duration / 1000)}
        </div>
