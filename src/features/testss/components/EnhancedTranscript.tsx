...
import type { TranscriptData, TranscriptSegment, WordPronunciation } from '../types/pronunciation.types';
...
interface EnhancedTranscriptProps {
  ...
  onWordAnalysisRequest?: (word: string) => void;
  onSegmentComplete?: (segmentId: string, metrics: SegmentMetrics) => void;
  className?: string;
  features?: {
    showSegmentPlayButtons?: boolean;
...
const TranscriptSegmentComponent: React.FC<TranscriptSegmentProps> = ({
  ...
  const handleWordClick = useCallback((word: WordPronunciation, event: React.MouseEvent) => {
  ...
  const currentWordIndex = segment.words.findIndex(word => 
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span className="font-mono">{formatTimeFromMs(segment.startTime)}</span>
          </button>
          
          {/* Segment accuracy */}
          <div className="text-sm">
            <span className={`px-2 py-1 rounded text-xs font-medium ${
              segment.overallAccuracy >= 90 ? 'bg-green-100 text-green-700' :
              segment.overallAccuracy >= 70 ? 'bg-blue-100 text-blue-700' :
              segment.overallAccuracy >= 50 ? 'bg-yellow-100 text-yellow-700' :
        ...
        <div className="text-xs text-gray-500">
          {segment.words.length} words • {formatTimeFromMs(segment.endTime - segment.startTime)} duration
        </div>
      </div>
...
const EnhancedTranscript: React.FC<EnhancedTranscriptProps> = ({
  ...
  const handleTimeUpdate = useCallback((time: number) => {
    ...
    if (currentSegment && currentSegment.id !== activeSegmentId) {
      ...
      if (mergedFeatures.autoScrollToCurrentSegment && transcriptRef.current) {
  ...
  }, [activeSegmentId, transcriptData.segments, mergedFeatures.autoScrollToCurrentSegment]);
  ...
  const overallStats = React.useMemo(() => {
    const totalWords = transcriptData.segments.reduce((sum, segment) => sum + segment.words.length, 0);
    const totalErrors = transcriptData.segments.reduce(
      (sum, segment) => sum + segment.words.reduce((wordSum, word) => wordSum + word.errors.length, 0), 
      0
    );
    const averageAccuracy = transcriptData.segments.reduce(
      (sum, segment) => sum + segment.overallAccuracy, 0
    ) / transcriptData.segments.length;
    
    // Calculate error type breakdown
    const errorBreakdown = transcriptData.segments.reduce((breakdown, segment) => {
  ...
  return (
    <div className={`bg-white rounded-xl border-2 border-purple-100 overflow-hidden ${className}`}>
      {/* Audio Player */}
      <EnhancedAudioPlayer
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 border-b border-purple-100">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="text-sm text-gray-600">
            {transcriptData.metadata.language} • {transcriptData.metadata.assessmentType}
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 text-center">
          <div className="bg-white rounded p-2">
            <div className="text-xl font-bold text-blue-600">{overallStats.averageAccuracy}%</div>
            <div className="text-xs text-gray-600">Overall Accuracy</div>
          </div>
          <div className="bg-white rounded p-2">
            <div className="text-xl font-bold text-green-600">{overallStats.totalWords}</div>
            <div className="text-xs text-gray-600">Total Words</div>
          </div>
          <div className="bg-white rounded p-2">
            <div className="text-xl font-bold text-orange-600">{overallStats.totalErrors}</div>
            <div className="text-xs text-gray-600">Issues Found</div>
          </div>
          <div className="bg-white rounded p-2">
            <div className="text-xl font-bold text-purple-600">{overallStats.totalSegments}</div>
        ...
        {overallStats.totalErrors > 0 && (
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(overallStats.errorBreakdown).map(([errorType, count]) => (
                <div key={errorType} className="bg-white rounded px-2 py-1 border border-gray-200">
                  <span className={`text-xs font-medium ${
                    errorType === 'mispronunciation' ? 'text-orange-600' :
                    errorType === 'omission' ? 'text-red-600' :
                    errorType === 'insertion' ? 'text-purple-600' :
                    errorType === 'unexpected_break' ? 'text-blue-600' :
                    errorType === 'missing_break' ? 'text-green-600' :
                    errorType === 'monotone' ? 'text-gray-600' :
                    'text-gray-600'
                  }`}>
      ...
      <div className="bg-gray-50 p-4 border-t border-gray-200">
        <div className="text-sm text-gray-600">
          ...
          {/* Color Legend */}
          <div className="mb-3">
            <div className="text-xs font-medium text-gray-700 mb-1">Accuracy Colors:</div>
            <div className="flex flex-wrap gap-4 text-xs">
              <span><span className="text-green-700 font-medium">●</span> Excellent (90%+)</span>
              <span><span className="text-blue-700 font-medium">●</span> Good (70-89%)</span>
              <span><span className="text-yellow-600 font-medium">●</span> Fair (50-69%)</span>
              <span><span className="text-red-600 font-medium">●</span> Needs Practice (&lt;50%)</span>
              <span><span className="text-purple-600 font-medium">ˈ</span> Stressed word</span>
            </div>
          </div>

          {/* Error Types Legend */}
          <div className="mb-3">
            <div className="text-xs font-medium text-gray-700 mb-1">Error Types:</div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
              <span><span className="text-orange-600">🔄</span> Mispronunciation</span>
              <span><span className="text-red-600">❌</span> Omission</span>
              <span><span className="text-purple-600">➕</span> Insertion</span>
              <span><span className="text-blue-600">⏸️</span> Unexpected break</span>
              <span><span className="text-green-600">⏩</span> Missing break</span>
              <span><span className="text-gray-600">📏</span> Monotone</span>
            </div>
          </div>

          <div className="text-xs text-gray-500">
            Click any word to jump to that moment • Click words for detailed pronunciation analysis
          </div>
        </div>
      </div>
    </div>
