// Demo page to showcase all pronunciation assessment features
import { useState } from 'react';
import EnhancedTranscript from './EnhancedTranscript';
import { 
  createMockTranscriptData, 
  createMockDataWithErrors, 
  createExcellentPronunciationData 
} from '../data/mockPronunciationData';

const PronunciationDemo = () => {
  const [selectedDemo, setSelectedDemo] = useState<'normal' | 'errors' | 'excellent'>('normal');
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg border p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <h3 className="font-medium text-purple-600">Pronunciation Details</h3>
            <ul className="text-sm text-gray-600 space-y-1">
      ...
      <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
        ...
        <div className="text-sm text-blue-700 space-y-2">
          <p><strong>🎯 Click any word</strong> to jump to that moment in the audio</p>
          <p><strong>🎨 Color meanings:</strong> Green = Excellent, Blue = Good, Yellow = Fair, Red = Needs Practice</p>
          <p><strong>⭐ Bold words</strong> with accent marks are stressed in the sentence</p>
          <p><strong> Click words</strong> for simple pronunciation analysis popup</p>
          <p><strong>📊 Segment accuracy</strong> shows overall performance for each sentence</p>
        </div>
      </div>

      {/* Current Demo Info */}
      <div className="bg-gray-50 rounded-lg border p-4">
        <h2 className="text-lg font-semibold mb-3">Current Demo: 
          <span className={`ml-2 ${
            selectedDemo === 'excellent' ? 'text-green-600' :
            selectedDemo === 'errors' ? 'text-red-600' : 'text-blue-600'
          }`}>
            {selectedDemo === 'excellent' ? 'Excellent Performance' :
             selectedDemo === 'errors' ? 'Common Errors' : 'Mixed Performance'}
          </span>
        </h2>
      ...
      <div className="bg-gray-50 rounded-lg border p-4">
        ...
        <div className="text-sm text-gray-600 space-y-2">
          <p><strong>Components:</strong> EnhancedAudioPlayer, PronunciationWord, PronunciationPopup, EnhancedTranscript</p>
          <p><strong>Data Structure:</strong> Supports Azure Pronunciation Assessment format with simplified display</p>
          <p><strong>Features:</strong> Reusable components, customizable features, TypeScript support, responsive design</p>
          <p><strong>UI:</strong> Clean and simple interface focused on essential pronunciation feedback</p>
        </div>
      </div>
    </div>
  );
