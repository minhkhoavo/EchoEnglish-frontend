...
const getErrorTypeIcon = (errorType: string): string => {
  switch (errorType) {
    ...
    case 'unexpected_break': return '⏸️';
    case 'missing_break': return '⏩';
...
const getErrorTypeColor = (errorType: string): string => {
  switch (errorType) {
    case 'mispronunciation': return 'text-orange-600';
    case 'omission': return 'text-red-600';
    case 'insertion': return 'text-purple-600';
    case 'unexpected_break': return 'text-blue-600';
    case 'missing_break': return 'text-green-600';
    case 'monotone': return 'text-gray-600';
...
const PronunciationPopup: React.FC<PronunciationPopupProps> = ({
  ...
  // Better positioning to avoid being cut off
  const popupWidth = 280;
  const popupHeight = 250; // Giảm chiều cao do đã bỏ learning tips
  const margin = 10;
  
  const left = Math.max(margin, Math.min(position.x - popupWidth / 2, window.innerWidth - popupWidth - margin));
  const top = position.y - popupHeight - 10 < margin 
    ? position.y + 20 // Show below if not enough space above
    : position.y - popupHeight - 10; // Show above by default
  ...
  return (
    <>
      ...
      {/* Popup */}
      <div
        className={`fixed z-50 bg-white rounded-lg shadow-xl border compact-popup pronunciation-popup ${className}`}
        style={{
          left: left,
      ...
      >
        ...
        <div className="bg-blue-500 text-white p-2 rounded-t-lg header">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-lg font-bold ${getAccuracyColor(wordData.accuracy).replace('text-', 'text-white bg-').replace('-500', '-600')} px-1 rounded`}>
                {wordData.accuracy}%
              </span>
            </div>
            {onClose && (
        ...
        <div className="p-3 text-xs overflow-y-auto" style={{ maxHeight: '180px' }}>
              ...
              <div className="space-y-1">
                {wordData.expectedPronunciation && (
                  <div className="flex items-center text-xs">
                    <span className="text-green-600 font-semibold mr-2 min-w-[60px]">Expected:</span>
                    <span className="font-mono bg-green-100 px-1 rounded">{wordData.expectedPronunciation}</span>
                  </div>
                )}
                {wordData.actualPronunciation && (
                  <div className="flex items-center text-xs">
                    <span className="text-blue-600 font-semibold mr-2 min-w-[60px]">Your said:</span>
                    <span className="font-mono bg-blue-100 px-1 rounded">{wordData.actualPronunciation}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="mb-3 bg-gray-50 p-2 rounded">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-gray-600">Overall Accuracy:</span>
              <span className={`text-lg font-bold ${getAccuracyColor(wordData.accuracy)}`}>
                {wordData.accuracy}%
              </span>
            </div>
            {wordData.confidenceScore && (
              <div className="flex items-center justify-between mt-1">
                ...
                <span className="text-xs text-gray-700">{wordData.confidenceScore}%</span>
              </div>
            )}
          </div>
                  ...
                  <div key={index} className="flex items-center justify-between text-xs bg-gray-50 p-2 rounded">
                    <div className="flex items-center space-x-2">
                      {phoneme.expectedPhoneme && phoneme.actualPhoneme ? (
                        <>
                          <span className="text-green-600 font-mono">/{phoneme.expectedPhoneme}/</span>
                          <span className="text-gray-400">→</span>
                          <span className="text-blue-600 font-mono">/{phoneme.actualPhoneme}/</span>
                        </>
                      ) : (
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {phoneme.isCorrect === false && <span className="text-red-600">✗</span>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-1">
                {wordData.errors.map((error, index) => (
                  <div key={index} className="flex items-center justify-between text-xs bg-red-50 p-2 rounded">
                    <div className="flex items-center space-x-2">
                      <span className="mr-1">{getErrorTypeIcon(error.type)}</span>
                      <span className={`capitalize ${getErrorTypeColor(error.type)}`}>
