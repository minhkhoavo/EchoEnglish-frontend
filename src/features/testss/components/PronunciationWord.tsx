...
import type { WordPronunciation, AccuracyLevel } from '../types/pronunciation.types';
...
const getAccuracyStyles = (level: AccuracyLevel, hasErrors: boolean): string => {
...
const getWordDisplayFormat = (wordData: WordPronunciation): { displayText: string; styles: string; prefix?: string } => {
  let displayText = wordData.word;
  let styles = '';
  let prefix = '';
  
  // Kiểm tra các error types để format hiển thị
  const hasOmission = wordData.errors.some(error => error.type === 'omission');
  const hasMissingBreak = wordData.errors.some(error => error.type === 'missing_break');
  const hasInsertion = wordData.errors.some(error => error.type === 'insertion');
  const hasUnexpectedBreak = wordData.errors.some(error => error.type === 'unexpected_break');
  const hasMonotone = wordData.errors.some(error => error.type === 'monotone');
  const hasMispronunciation = wordData.errors.some(error => error.type === 'mispronunciation');
  
  if (hasOmission) {
    // Omissions: từ bị bỏ qua hiển thị trong ngoặc vuông màu xám
    displayText = `[${wordData.word}]`;
    styles += ' text-gray-500 italic bg-gray-100 px-1 rounded';
  } else if (hasInsertion) {
    // Insertions: ký hiệu [+] màu hồng + từ thừa tô đỏ
    prefix = '[+] ';
    styles += ' text-red-600 bg-red-100 px-1 rounded';
  } else if (hasUnexpectedBreak) {
    // Unexpected break: vạch/dấu nhỏ màu hồng nhạt chen giữa
    prefix = '| ';
    styles += ' text-pink-600';
  } else if (hasMissingBreak) {
    // Missing break: ngoặc vuông rỗng [] màu xám
    prefix = '[] ';
    styles += ' text-gray-500 border-b border-gray-400 border-dashed';
  } else if (hasMonotone) {
    // Monotone: hiển thị đặc biệt
    styles += ' text-purple-600 bg-purple-50 px-1 rounded opacity-75';
  } else if (hasMispronunciation) {
    // Mispronunciation: chỉ màu cam, không cần background
    styles += ' text-orange-600';
  }
  
  // Duplicated words: thêm gạch ngang ở giữa (độc lập với error types)
...
const PronunciationWord: React.FC<PronunciationWordProps> = ({
  ...
  // Calculate styles và display text
  const accuracyLevel = getAccuracyLevel(wordData.accuracy);
  const hasErrors = wordData.errors.length > 0;
  
  // Lấy format hiển thị dựa trên error types
  const { displayText, styles: errorStyles, prefix } = getWordDisplayFormat(wordData);
  
  let wordStyles = className;
  
  // Thêm error-specific styles
  if (mergedFeatures.showErrorHighlight) {
    wordStyles += ' ' + errorStyles;
  }
  
  // Chỉ apply accuracy colors nếu không có error styles đặc biệt
  if (mergedFeatures.showAccuracyColors && !hasErrors) {
    ...
    // Apply accuracy color nếu có error nhưng không có error-specific styling
    wordStyles += ' ' + getAccuracyStyles(accuracyLevel, hasErrors);
  }
  
  if (mergedFeatures.showStressMarking && wordData.isStressed) {
    // Stress word styling using Tailwind - no overlap issues
    wordStyles += ' font-black text-lg leading-relaxed mx-1 px-1 inline-block tracking-wide';
    if (mergedFeatures.emphasizeStress) {
      wordStyles += ' font-extrabold'; // Chỉ làm đậm hơn, không có animation
    }
  }

  return (
    <>
      {/* Hiển thị prefix nếu có (cho insertion, unexpected break, missing break) */}
      {prefix && (
        <span className={`inline-block mr-1 text-xs font-mono ${
          prefix.includes('[+]') ? 'text-pink-500' :
          prefix.includes('|') ? 'text-pink-300' :
          prefix.includes('[]') ? 'text-gray-400' : ''
        }`}>
          {prefix}
        </span>
      )}
        ...
        >
          <div className="text-center">
            ...
            <div className={`text-sm font-medium ${
              ...
              accuracyLevel === 'good' ? 'text-blue-400' :
              accuracyLevel === 'fair' ? 'text-yellow-400' : 'text-red-400'
            }`}>
              {wordData.accuracy}%
            </div>
